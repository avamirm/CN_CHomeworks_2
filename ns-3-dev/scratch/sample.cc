...
void MyHeader::SetClientPort(uint16_t port)
{
    clientPort = port;
}

void MyHeader::SetClientIp(Ipv4Address ip)
{
    clientIp = ip;
}

uint16_t MyHeader::GetClientPort(void) const
{
    return clientPort;
}

Ipv4Address MyHeader::GetClientIp(void) const
{
    return clientIp;
}
...
void Mapper::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->Recv()))
    {
        ...
        if (it != mappings.end())
        {
            ...
            // destNewHeader.Print(std::cout);
            newSocket->Send(newPacket);
            cout << id << " - " << static_cast<char>(destNewHeader.GetData()) << endl;
            newSocket->Close();
        }
    }
}
...
Client::Client(uint16_t port, Ipv4InterfaceContainer &ip, uint16_t masterPort, Ipv4InterfaceContainer &masterIp)
...
void CheckTimeOut(Ptr<Socket> socket, uint16_t data, uint16_t port, Ipv4Address ip)
{
...
void Client::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->Recv()))
    {
        ...
        MyHeader destinationHeader;
        packet->RemoveHeader(destinationHeader);
        // cout << static_cast<char>(destinationHeader.GetData()) << endl; /////////////////////////////////////////////////////////////
    }
}

Master::Master(uint16_t port, array<uint16_t, MAPPERS_COUNT> mappersPorts, Ipv4InterfaceContainer &ip, Ipv4InterfaceContainer &mapperIp)
    : port(port), mappersPorts(mappersPorts),
      ip(ip), mapperIp(mapperIp)
{
    std::srand(time(0));
}
...
void Master::StartApplication(void)
{
    ...
    {
        mapperSocket[i] = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
        InetSocketAddress mapperAddr = InetSocketAddress(mapperIp.GetAddress(i), mappersPorts[i]);
        mapperSocket[i]->Connect(mapperAddr);
    }
...
void Master::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->Recv()))
    {
        ...
        for (int i = 0; i < MAPPERS_COUNT; i++)
        {
            mapperSocket[i]->Send(sendPacket);
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < MAPPERS_COUNT; i++)
    {
        ...
        // for (int j = 0; j < 8; j++)
        // {
        //     mappings.insert(std::pair<uint16_t, char>(j, 'a' + (j * (i + 1)) + 8));
        // }
